...
{
    [TestFixture]
    public class DistributedCachingProviderTests
    {
        [SetUp]
        ...
        private readonly DistributedCacheEntryOptions oneHourNonRemoveableDistributedCacheEntryOptions =
        ...
        {
            Action act = () => sut.Add("", new object(), new DistributedCacheEntryOptions());
            act.Should().Throw<ArgumentOutOfRangeException>();
        }
        ...
        {
            Action act = () => sut.Add(null, new object(), new DistributedCacheEntryOptions());
            act.Should().Throw<ArgumentNullException>();
        }
        ...
        {
            sut.Add(TestKey, "testObject", new DistributedCacheEntryOptions());
            Assert.AreEqual("testObject", sut.Get<string>(TestKey));
        }
        ...
        {
            sut.DistributedCacheProvider.Should().NotBeNull();
        }
        ...
        public void DefaultContructorThenGetOrAddFromSecondCachingServiceHasSharedUnderlyingCache()
        {
            ...
            IDistributedCache memoryDistributedCache = new MemoryDistributedCache(Options.Create(options));
            IDistributedCacheProvider distributedCacheProvider = new DistributedCacheProvider(memoryDistributedCache);
            var cacheOne = new DistributedCachingService(distributedCacheProvider);
            var cacheTwo = new DistributedCachingService(distributedCacheProvider);
        ...
        public async Task GetOrAddAyncAllowsCachingATask()
        {
            ...
            var actualResult =
                await sut.GetOrAddAsync(TestKey, FetchAsync, oneHourNonRemoveableDistributedCacheEntryOptions);
        ...
        public void GetOrAddWithPolicyAndThenGetValueObjectReturnsCorrectType()
        {
            ...
            sut.GetOrAdd(TestKey, Fetch, oneHourNonRemoveableDistributedCacheEntryOptions);
            var actual = sut.Get<int>(TestKey);
            Assert.AreEqual(123, actual);
        }
